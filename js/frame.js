...
if (!userAgent.match(/safari/i)) {
  for (let i = 1; i < 170; i++) {
    structureImages.push(`0${`00${i}`.slice(-3)}.webp`);
  }
  for (let p = 1; p < 338; p++) {
    planeteImages.push(`0${`00${p}`.slice(-3)}.webp`);
  }
} else {
  for (let i = 1; i < 170; i++) {
    structureImages.push(`0${`00${i}`.slice(-3)}.png`);
  }
  for (let p = 1; p < 338; p++) {
    planeteImages.push(`0${`00${p}`.slice(-3)}.png`);
  }
}

if (langue == "en") {
  loadImages("image/planete/", planeteImages);
  loadImages("image/structureSM/", structureImages);
} else {
  loadImages("../image/planete/", planeteImages);
  loadImages("../image/structureSM/", structureImages);
}
...
class Canvas {
  ...
  renderIndex(e) {
    ...
    // Trouver l'image chargée la plus proche
   ...
   *
   * Si l'image et le contexte sont les seuls arguments, le rectangle sera égal au canevas.
   */
  drawImageCover(ctx, img, x, y, w, h, offsetX, offsetY) {
    ...
    // le décalage par défaut est le centre
    offsetX = typeof offsetX === "number" ? offsetX : 0.5;
    offsetY = typeof offsetY === "number" ? offsetY : 0.5;

    // garder les limites [0.0, 1.0]
...
var langue = $("select#site_langue").val();
if (langue == "en") {
  const StructureSequence = new ScrollSequence({
    container: ".structure-sequence",
    scrollWith: ".structure-container",
    images: structureImages,
    imagesRoot: "/image/structureSM/",
    // priorityFrames: [0, 20, 40, 60, 90],
    priorityFrames: [0, 40, 80, 120],
    cover: true,
  ...
  const MapSequence = new ScrollSequence({
    container: ".maps-sequence",
    scrollWith: ".maps-container",
    images: planeteImages,
    imagesRoot: "/image/planete/",
    priorityFrames: [0, 20, 40, 60, 90],
    cover: true,
    playUntil: "scroll-out",
    starts: "in",
...
var speak_contain = document.querySelectorAll(".speak");
if (speak_contain.length) {
  ScrollOut({
    targets: ".speak",
    cssProps: {
      viewportY: true,
