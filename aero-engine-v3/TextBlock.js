...
export class TextBlock extends HTMLElement {
    ...
    constructor() {
        ...
        let val;
        this.type = (val = this.getAttribute("type")) ? val : "std";
        this.setAttribute("type", this.type);

        this.theme = (val = this.getAttribute("theme")) ? val : "light";
        this.setAttribute("theme", this.theme);

        this.arrangement = (val = this.getAttribute("arrangement")) ? val : "std";
        this.setAttribute("arrangement", this.arrangement);

        this.numbering = (val = this.getAttribute("numbering")) ? val : "none";

        /* <id> */
        if (this.hasAttribute("id")) { this.id = this.getAttribute("id"); }
        /* </id> */


        /* <background> */
        if (val = this.getAttribute("backgroundColor")) {
            switch (val) {
                case "black": this.classList.add("background-black"); break;
                case "white": this.classList.add("background-white"); break;
                case "grey64": this.classList.add("background-grey64"); break;
                case "grey128": this.classList.add("background-grey128"); break;
                case "grey192": this.classList.add("background-grey192"); break;
            }
        }
        ...
        while (node) {
            ...
            switch (type) {
                case "h1": {
                    numbering.h1Number++;
                    this.elements.push(new TxBkHeader1(node, numbering));
                    numbering.h2Number = 0;
                } break;

                case "h2": {
                    numbering.h2Number++;
                    this.elements.push(new TxBkHeader2(node, numbering));
                    numbering.pNumber = 0;
                } break;

                case "p": this.elements.push(new TxBkParagraph(node)); break;
                case "svg": this.elements.push(new TxBkSVG(node)); break;
                case "snippet": this.elements.push(new TxBkCodeSnippet(node)); break;
                case "ol": this.elements.push(new TxOrderedBkList(node)); break;
                case "ul": this.elements.push(new TxUnorderedBkList(node)); break;
                case "links": this.elements.push(new TxBkLinks(node)); break;
            }
        ...
        this.elements.forEach(element => this.appendChild(element.html_getNode()));
...
export class TxBkLinks extends TextBlockElement {
    ...
    constructor(sources) {
        ...
        while (node) {
            let type = node.nodeName.toLowerCase();
            switch (type) {
                case "a": this.elements.push(new TxBkAnchor(node)); break;
            }
            node = node.nextSibling;
        }
